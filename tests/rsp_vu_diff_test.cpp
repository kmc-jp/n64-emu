...
uint32_t encode_vu(int funct, int vd, int vs, int vt, int element) {
    // COP2 encode: element in bits 21..24, vt 16..20, vs 11..15, vd 6..10, funct 0..5
...
const int kSimdFuncts[] = {
    ...
    0x10, 0x11, 0x13, 0x14, 0x15, 0x20, 0x21, 0x22, 0x23, 0x27, 0x28, 0x29,
    0x2A, 0x2B, 0x2C, 0x2D,
};
    ...
    for (int element = 0; element < 16; element++) {
        for (int dest = 0; dest < 8; dest++) {
            ...
            if (scalar.vreg(0).lane(dest) !=
                ...
                std::fprintf(stderr, "broadcast expect failed element=%d dest=%d\n",
                             element, dest);
                return 1;
            }
            std::string why;
